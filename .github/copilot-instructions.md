...
**Key files (quick map)**
- `script.js` — desktop scrolling/table renderer. Uses `fetch('display.csv', {cache: 'no-store'})`, skips first 3 lines with `.split('\n').slice(3)`, and treats column 0 as a flag (`'X'` => executed).
- `public/mobile-script.js` — mobile card UI. Also skips first 3 rows and maps CSV columns to `{id,name,song,author,choreographer}`. Speed is pixels/second (px/s).
- `index.html` / header — contains `#next-coreo` UI element and includes `script.js`.
- `display.csv` (root) and `public/display.csv` (deployed) — keep them in sync before deploying.
- `NextCoreo.csv` (root & `public/`) — used to show a single label (cell A1). Code strips BOMs and uses cache-busting query strings.
...
- Fullscreen handling: both `script.js` and `public/mobile-script.js` implement `toggleFullscreen()` and restart scrolling on fullscreen changes — preserve this to avoid visual jumps.
...
**Editing data or CSV format — checklist**
...
- Animation math differences (desktop vs mobile) — update both implementations.
