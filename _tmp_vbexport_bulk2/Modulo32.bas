...
Sub EstraiBraniOrdinati()
    ...
    For i = LBound(criteri) To UBound(criteri)
        For riga = 2 To ultimaRigaLista ' Presume che la riga 1 sia l'intestazione
...
Sub AggiungiBranoOrdinato(wsRisultati As Worksheet, ByRef ultimaRigaRisultati As Long, brano As String, criterio As String)
    ...
    ' Controlla se il brano è già presente nel foglio "Risultati"
    trovato = False
    For Each cella In wsRisultati.Range("A1:A" & ultimaRigaRisultati)
    ...
    ' Se il brano non è già stato estratto, aggiungilo al foglio "Risultati"
    If Not trovato Then
