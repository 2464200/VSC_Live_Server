...
' === CONFIGURAZIONE ===
Private Const SHEET_NAME As String = "border�"
...
Public Sub EsportaBorderoCSV()
    ...
    lastRowData = LastRowBorder()
    If lastRowData < 12 Then
        MsgBox "Nessun dato disponibile da esportare.", vbInformation
        Exit Sub
    End If
    header = ws.Range("A11:D11").Value2
    data = ws.Range("A12:D" & lastRowData).Value2
...
' Padding a 3 cifre:
' - Se il valore � numerico intero (es. 12 o 12,0) ? "012", "012"
' - Mantiene gli zeri iniziali gi� presenti (es. "7" ? "007"; "045" resta "045")
' - Se � vuoto ? stringa vuota
...
' Costruisce una riga CSV da un array di stringhe gi� pronte (1..4)
...
' Verifica se la riga i (tra le colonne firstCol..lastCol) � completamente vuota
