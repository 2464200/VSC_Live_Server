...
Sub CopiaERifila(criterio As String)
    ...
    Set rng1 = wsElencoBrani.Range("$B$2:$D$612")
    rng1.Copy Destination:=wsLista.Range("$B$8")

    ' Copia il secondo intervallo
    Set rng2 = wsElencoBrani.Range("$H$2:$J$612")
    rng2.Copy Destination:=wsLista.Range("$E$8")

    ' Inizializza il Dictionary
    Set dict = CreateObject("Scripting.Dictionary")
    
    ' Leggi i dati dall'intervallo "D8:D618" del foglio Lista per serata
    Set rng1 = wsLista.Range("$D$8:$D$618")
...
Sub Lista_SUPERAVANZATO_2()
    CopiaERifila "SUPER AVANZATO 1+2"
End Sub
