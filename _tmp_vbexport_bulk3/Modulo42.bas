...
Sub EsportaDatiInWord_101_Corretto()
    ...
    ' --- PRIORITY 1: Cerca valoreD7 nella colonna D (D12 in gi?) ---
    If Not IsEmpty(valoreD7) Then ' Cerca solo se D7 non ? vuoto
    ...
    ' --- PRIORITY 2: Se valoreD7 non ? stato trovato (o era vuoto), cerca valoreD9 nella colonna C (C12 in gi?) ---
    If rigaTrovata = 0 And Not IsEmpty(valoreD9) Then ' Cerca D9 solo se D7 non ha dato risultati E D9 non ? vuoto
    ...
    If rigaTrovata = 0 Then
        MsgBox "Nessun valore corrispondente trovato n? per D7 (in colonna D) n? per D9 (in colonna C).", vbExclamation
        Exit Sub
    Else
        testoColonnaN = ws.cells(rigaTrovata, "N").Value
        
        ' DECISIONE: Quale valore usare come intestazione/parte in grassetto in Word?
        ' Usa il valore di D7 se D7 non ? vuoto.
        ' Altrimenti (se D7 era vuoto o non trovato, e la riga ? stata trovata tramite D9),
        ' usa il valore della colonna D della riga trovata.
        If Not IsEmpty(valoreD7) Then
            valoreIntestazioneWord = valoreD7
        Else ' La riga ? stata trovata grazie a D9 (o comunque D7 era vuoto)
    ...
    messaggioFinale = "Esportazione completata! Il testo ? stato copiato in un nuovo documento Word." & vbCrLf & vbCrLf & _
